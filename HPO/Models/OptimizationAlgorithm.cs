...
public class OptAlgorithm
{
    ...
    public Dictionary<double, AssetSpecifications> GetObjective(List<AssetSpecifications> boilers, int[] par, double ElectricityPrice)
    {
        List<AssetSpecifications> Boilers = boilers.Where(n => n.IsActive == true).ToList();
        List<AssetSpecifications> HeatPumps = Boilers.Where( n => n.UnitType == "Motor").ToList();
        foreach(var hp in HeatPumps)
        {
            hp.ProductionCost += ElectricityPrice;
        }
        List<AssetSpecifications> GasMotors =  Boilers.Where( n => n.UnitType == "Heat Pump").ToList();
    ...
    public void CalculateElectricity(List<AssetSpecifications> HeatPumps, List<AssetSpecifications> GasMotors, double ElectricityPrice)
    {
